...
public class TeacherCourseModuleController {
	...
	public HashMap<String,Object> assignHomework(@RequestBody Map<String,Object> request){
		...
		//none rank
		//action
		String studyId=String.valueOf(UUID.randomUUID());
		try {
			this.studyDao.assignHomework(studyId,paperId,classId,assignTeacherId,deadLine,submitNumber,assignTime,paperType,examTime);
		}catch(Exception e) {
			e.printStackTrace();
			status="-2";
			result.put("result", status);
